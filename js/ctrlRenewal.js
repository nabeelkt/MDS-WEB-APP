 var elemFocus = false;
 app.controller('ctrlRenewal', ['$scope', '$http', '$window', 'Upload', '$timeout', '$filter', function($scope, $http, $window, Upload, $timeout, $filter) {
     ...
     scope.btnSubmit = function() {
         ...
         console.log($filter('date')($.trim($('#dtptvto').val(), 'dd/MM/yyyy')))
         scope.msg = '';
     ...
     $scope.SetDataCustomer = function() {
         ...
         $.ajax({
             url: "GService.asmx/fn_GetLIcenceInfo",
             type: "GET",
             data: {
                 ls_euid: $window.sessionStorage.getItem('EUID'),
                 ai_cusid: scope.customer.id
             },
             dataType: "json",
             headers: {
                 'Content-Type': 'application/x-www-form-urlencoded'
